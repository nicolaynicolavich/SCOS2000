...
class IntegerAttributeOutOfRangeError(RunTimeError):
	"""To be raised when run-time constraints are checked. Only appliccable to integers."""
	def __init__(self, field, value):
		super().__init__(field, value)
...
class UnmatchingForeignKeyError(RunTimeError):
	...
	def __init__(self, field, value, foreign_key, foreign_key_table):
		super().__init__(field, value)
		self.message= "Field %s ('%s') does not match any value in column %s at %s" % (str(field), str(value), str(foreign_key), str(foreign_key_table))
...
def getColumnNames(class_type):
	...
	wether the attribute is nullable and a dictionary with the attribute's foreign key.
	Input class_type as a class.
	Output: The foreign key dictionary ontains a key for each column of the table at hand. If that coulumn
...
def checkRuntimeConstraints(object_to_update, class_type_noformat, class_attributes, object_attributes, session, override_attribute_dict):
	...
	attributes_dictionary= createAttributeDictionary(class_type_str, class_attributes, object_attributes) # Creates a dictionary with the attributes
	...
	if class_type_str == "DPF":
		...
		if (int(attributes_dictionary["GPF_UPUN"])< 1 or int(attributes_dictionary["GPF_UPUN"])> 99) and int(attributes_dictionary["GPF_UPUN"]) != 0:
		...
		if (int(attributes_dictionary["CCF_TYPE"]) < 0 or int(attributes_dictionary["CCF_TYPE"]) > 255) and attributes_dictionary["CCF_TYPE"] != 0:
			IntegerAttributeOutOfRange("CCF_TYPE", attributes_dictionary["CCF_TYPE"])
		if (int(attributes_dictionary["CCF_STYPE"]) < 0 or int(attributes_dictionary["CCF_STYPE"]) > 255) and attributes_dictionary["CCF_STYPE"] != 0:
			IntegerAttributeOutOfRange("CCF_STYPE", attributes_dictionary["CCF_STYPE"])
		if (int(attributes_dictionary["CCF_APID"]) < 0 or int(attributes_dictionary["CCF_APID"]) > 65535) and attributes_dictionary["CCF_APID"] != 0:
		...
		if attributes_dictionary["CCF_ILSCOPE"] not in ['G', 'L', 'S', 'B', 'F', 'T', 'N'] and attributes_dictionary["CCF_ILSCOPE"] != '':
			InvalidAttribute("CCF_ILSCOPE", attributes_dictionary["CCF_ILSCOPE"], not_applicable= False)
		if attributes_dictionary["CCF_ILSTAGE"] not in ['R', 'U', 'O', 'A', 'C'] and attributes_dictionary["CCF_ILSTAGE"] != '':
			InvalidAttribute("CCF_ILSTAGE", attributes_dictionary["CCF_ILSTAGE"], not_applicable= False)
		if (int(attributes_dictionary["CCF_SUBSYS"]) < 1 or int(attributes_dictionary["CCF_SUBSYS"]) > 255) and attributes_dictionary["CCF_SUBSYS"] != 0:
		...
		if (int(attributes_dictionary["CCF_MAPID"]) < 0 or int(attributes_dictionary["CCF_MAPID"]) > 63) and attributes_dictionary["CCF_MAPID"] != 0:
			IntegerAttributeOutOfRange("CCF_MAPID", attributes_dictionary["CCF_MAPID"])
		if (int(attributes_dictionary["CCF_RAPID"]) < 1 or int(attributes_dictionary["CCF_RAPID"]) > 65535) and attributes_dictionary["CCF_RAPID"] != 0:
			IntegerAttributeOutOfRange("CCF_RAPID", attributes_dictionary["CCF_RAPID"])
		if (int(attributes_dictionary["CCF_ACK"]) < 0 or int(attributes_dictionary["CCF_ACK"]) > 15) and attributes_dictionary["CCF_ACK"] != 0:
		...
		elif (int(attributes_dictionary["CCF_SUBSCHEDID"]) < 1 or int(attributes_dictionary["CCF_SUBSCHEDID"]) > 65535) and int(attributes_dictionary["CCF_SUBSCHEDID"]) != 0:
		...
		if (int(attributes_dictionary["CSF_SUBSYS"]) < 1 or int(attributes_dictionary["CSF_SUBSYS"]) > 255) and attributes_dictionary["CSF_SUBSYS"] != 0:
			IntegerAttributeOutOfRange("CSF_SUBSYS", attributes_dictionary["CSF_SUBSYS"])
		if (int(attributes_dictionary["CSF_SUBSCHEDID"]) < 1 or int(attributes_dictionary["CSF_SUBSCHEDID"]) > 65535) and attributes_dictionary["CSF_SUBSCHEDID"] != 0:
		...
		if attributes_dictionary["CSS_ILSCOPE"] not in ['G', 'L', 'S', 'B', 'F', 'T', 'N'] and attributes_dictionary["CSS_ILSCOPE"] != '':
			InvalidAttribute("CSS_ILSCOPE", attributes_dictionary["CSS_ILSCOPE"], not_applicable= False)
		if (attributes_dictionary["CSS_BLOCK"] != '' or attributes_dictionary["CSS_GROUP"] != '') and attributes_dictionary["CSS_ILSCOPE"] in ['G', 'L', ''] and (attributes_dictionary["CSS_BLOCK"] != 'E' or attributes_dictionary["CSS_GROUP"] != 'E'):
			InvalidAttribute("CSS_ILSCOPE", attributes_dictionary["CSS_ILSCOPE"], not_applicable= False)
		if attributes_dictionary["CSS_ILSTAGE"] not in ['R', 'U', 'O', 'A', 'C'] and attributes_dictionary["CSS_ILSTAGE"] != '':
		...
		if attributes_dictionary["DST_ROUTE"].count('.') not in [0, 1, 2]:
			...
			if DST_ROUTE_params[0] not in ["NCTRS", "EGSE"]:
				InvalidAttribute("DST_ROUTE", attributes_dictionary["DST_ROUTE"], not_applicable= False)
			elif DST_ROUTE_params[0] == "NCTRS" and (attributes_dictionary["DST_APID"] < 0 or attributes_dictionary["DST_APID"] > 2047):
				InvalidAttribute("DST_ROUTE", attributes_dictionary["DST_ROUTE"], not_applicable= False)
			# second parameter
		...
		if (int(attributes_dictionary["PID_PI1_VAL"]) < 0 or int(attributes_dictionary["PID_PI1_VAL"]) > (pow(2,31)-1)) and int(attributes_dictionary["PID_PI1_VAL"]) != 0:
			IntegerAttributeOutOfRange("PID_PI1_VAL", attributes_dictionary["PID_PI1_VAL"])
		if (int(attributes_dictionary["PID_PI2_VAL"]) < 0 or int(attributes_dictionary["PID_PI2_VAL"]) > (pow(2,31)-1)) and int(attributes_dictionary["PID_PI2_VAL"]) != 0:
		...
		if (int(attributes_dictionary["PID_TPSD"]) < 1 or int(attributes_dictionary["PID_TPSD"]) > (pow(2,31)-1) or int(attributes_dictionary["PID_TPSD"]) == -1) and attributes_dictionary["PID_TPSD"] != 0 and int(attributes_dictionary["PID_TPSD"]) != -1:
		...
		if int(attributes_dictionary["CVS_INTERVAL"]) < -1 and int(attributes_dictionary["CVS_INTERVAL"]) != 0: # second constraint is redundant but it is written for the sake of homogeinity
			IntegerAttributeOutOfRange("CVS_INTERVAL", attributes_dictionary["CVS_INTERVAL"])
		...
		if int(attributes_dictionary["VPD_POS"]) < -1 and int(attributes_dictionary["VPD_POS"]) != 0: # second constraint is redundant but it is written for the sake of homogeinity 
			IntegerAttributeOutOfRange("VPD_POS", attributes_dictionary["VPD_POS"])
		...
		if int(attributes_dictionary["VPD_DCHAR"]) not in [0, 1, 2] and int(attributes_dictionary["VPD_DCHAR"]) != 0: # second constraint is redundant but it is written for the sake of homogeinity 
			IntegerAttributeOutOfRange("VPD_DCHAR", attributes_dictionary["VPD_DCHAR"])
		if attributes_dictionary["VPD_FORM"] not in ['B', 'O', 'D', 'H', 'N'] and attributes_dictionary["VPD_FORM"] != '':
			InvalidAttribute("VPD_FORM", attributes_dictionary["VPD_FORM"], not_applicable= False)
		if (int(attributes_dictionary["VPD_OFFSET"]) < -32768 or int(attributes_dictionary["VPD_OFFSET"]) > 32767) and int(attributes_dictionary["VPD_OFFSET"]) != 0: # second constraint is redundant but it is written for the sake of homogeinity 
		...
		if attributes_dictionary["PCF_CURTX"] != '':
			if attributes_dictionary["PCF_CATEG"] == 'S':
				...
				if attributes_dictionary["PCF_CURTX"] not in CAF_attributes or attributes_dictionary["PCF_CURTX"] not in MCF_attributes or attributes_dictionary["PCF_CURTX"] not in LGF_attributes:
					UnmatchingForeignKey('PCF_CURTX', attributes_dictionary["PCF_CURTX"], 'CAF_NUMBR or MCF_IDENT or LGF_IDENT', 'CAF_table/ MCF_table/ LGF_table')
			elif attributes_dictionary["PCF_CATEG"] == 'T' or int(attributes_dictionary["PCF_PTC"]) in [7, 8, 9, 10]: # field must be left NULL:
				print("PCF_CURTX changed to NULL")
				object_to_update.PCF_CURTX = None
		if attributes_dictionary["PCF_CURTX"] == '' and attributes_dictionary["PCF_CATEG"] == 'S': # field cannot be left NULL
			InvalidAttribute("PCF_CURTX", attributes_dictionary["PCF_CURTX"], not_applicable= False)
		# Other attributes
		if attributes_dictionary["PCF_NAME"][0:3] == "VAR" or attributes_dictionary["PCF_NAME"][0:4] == "GVAR" or attributes_dictionary["PCF_NAME"][0] == "$":
			InvalidAttribute("PCF_NAME", attributes_dictionary["PCF_NAME"], not_applicable= False)
		if (int(attributes_dictionary["PCF_PID"]) < 0 or int(attributes_dictionary["PCF_PID"]) > (pow(2, 31)-1)) and int(attributes_dictionary["PCF_PID"]) != 0:
		...
		if attributes_dictionary["PCF_USCON"] not in ['N', 'Y'] and attributes_dictionary["PCF_USCON"] != '': # field must be corrected later (when OCP is available)
		...
		elif attributes_dictionary["PCF_NATUR"] == 'C' and attributes_dictionary["PCF_PARVAL"] == '': # field cannot be left NULL
		...
		if PCF_object == None:
			...
			if PCF_object.PCF_CATEG == 'N' and attributes_dictionary["OCP_TYPE"] in ['S', 'H', 'E'] and attributes_dictionary["OCP_LVALU"] == '':
				InvalidAttribute("OCP_LVALU", attributes_dictionary["OCP_LVALU"], not_applicable= False)
			if PCF_object.PCF_CATEG == 'S' and attributes_dictionary["OCP_TYPE"] in ['S', 'H'] and attributes_dictionary["OCP_LVALU"] == '':
				InvalidAttribute("OCP_LVALU", attributes_dictionary["OCP_LVALU"], not_applicable= False)
		# OCP_HVALU
		PCF_object= session.query(schema.PCF).filter_by(PCF_NAME= attributes_dictionary["OCP_NAME"]).first()
		if PCF_object == None:
		...
		if (int(attributes_dictionary["PLF_NBOCC"]) < 1 or int(attributes_dictionary["PLF_NBOCC"]) > 9999) and int(attributes_dictionary["PLF_NBOCC"]) != 0:
			IntegerAttributeOutOfRange("PLF_NBOCC", attributes_dictionary["PLF_NBOCC"])
		if (int(attributes_dictionary["PLF_LGOCC"]) < 0 or int(attributes_dictionary["PLF_LGOCC"]) > 32767) and int(attributes_dictionary["PLF_LGOCC"]) != 0:
			IntegerAttributeOutOfRange("PLF_LGOCC", attributes_dictionary["PLF_LGOCC"])
		if (int(attributes_dictionary["PLF_TIME"]) < -4080000 or int(attributes_dictionary["PLF_TIME"]) > 4080000) and int(attributes_dictionary["PLF_TIME"]) != 0:
			IntegerAttributeOutOfRange("PLF_TIME", attributes_dictionary["PLF_TIME"])
		if (int(attributes_dictionary["PLF_TDOCC"]) < 1 or int(attributes_dictionary["PLF_TDOCC"]) > 4080000) and int(attributes_dictionary["PLF_TDOCC"]) != 0:
		...
		if attributes_dictionary["SPC_FORM"] not in ['B', 'O', 'D', 'H', 'N'] and attributes_dictionary["SPC_FORM"] != '':
			InvalidAttribute("SPC_FORM", attributes_dictionary["SPC_FORM"], not_applicable= False)
		if attributes_dictionary["SPC_BACK"] not in ['0', '1', '2', '3', '4', '5', '6', '7'] and attributes_dictionary["SPC_FORM"] != '':
		...
		if attributes_dictionary["PCDF_TYPE"] not in ['A', 'T', 'S', 'K', 'P'] and attributes_dictionary["PCDF_TYPE"] != '': 
		...
		for CSF_object in CSF_rows:
			CSF_attributes.append(CSF_object.CSF_NAME)
		if attributes_dictionary["PSM_NAME"] not in CCF_attributes or attributes_dictionary["PSM_NAME"] not in CSF_attributes:
			UnmatchingForeignKey('PSM_NAME', attributes_dictionary["PSM_NAME"], 'CCF_CNAME or CSF_NAME', 'CCF_table and CSF_table')
		# PSM_PARSET
		PST_rows= session.query(schema.PST).all() # gets the PST table
		PST_attributes= []
		for PST_object in PST_rows:
		...
		if PCF_object == None:
			...
			if attributes_dictionary["PTV_INTER"] == 'E' and PCF_object.PCF_CURTX == None:
				InvalidAttribute("PTV_INTER", attributes_dictionary["PTV_INTER"], not_applicable= False)
		...
		if GPF_object == None:
			...
			if GPF_object.GPF_TYPE == 'Q' and attributes_dictionary["GPC_WHERE"] not in ['1', '2', '3', '4']:
				InvalidAttribute("GPC_WHERE", attributes_dictionary["GPC_WHERE"], not_applicable= False)
			if GPF_object.GPF_TYPE == 'S' and attributes_dictionary["GPC_WHERE"] not in ['1', '2', '3', '4', '5', '6', '7', '8']:
				InvalidAttribute("GPC_WHERE", attributes_dictionary["GPC_WHERE"], not_applicable= False)
		# GPC_NAME
		PCF_rows= session.query(schema.PCF).all() # gets the PCF table
		PCF_attributes= []
		...
		if attributes_dictionary["GPC_SYMB0"] not in ['0', '1', '2', '3', '4', '5', '6'] and attributes_dictionary["GPC_SYMB0"] != '':
			InvalidAttribute("GPC_SYMB0", attributes_dictionary["GPC_SYMB0"], not_applicable= False)
		if attributes_dictionary["GPC_LINE"] not in ['0', '1', '2', '3', '4', '5'] and attributes_dictionary["GPC_LINE"] != '':
			InvalidAttribute("GPC_LINE", attributes_dictionary["GPC_LINE"], not_applicable= False)
		if (int(attributes_dictionary["GPC_DOMAIN"]) < 0 or int(attributes_dictionary["GPC_DOMAIN"]) > pow(2, 16)-1) and int(attributes_dictionary["GPC_DOMAIN"]) != 0:
		...
		if PCF_object.PCF_CATEG == 'S':
			...
			for LGF_object in LGF_rows:
				LGF_attributes.append(LGF_object.LGF_IDENT)
			if attributes_dictionary["CUR_SELECT"] not in CAF_attributes or attributes_dictionary["CUR_SELECT"] not in MCF_attributes or attributes_dictionary["CUR_SELECT"] not in LGF_attributes:
				UnmatchingForeignKey('CUR_SELECT', attributes_dictionary["CUR_SELECT"], 'CAF_NUMBR or MCF_IDENT or LGF_IDENT', 'CAF_table/ MCF_table/ LGF_table')
		...
		for CSF_object in CSF_rows:
			CSF_attributes.append(CSF_object.CSF_NAME)
		if attributes_dictionary["CVP_TASK"] not in CCF_attributes or attributes_dictionary["CVP_TASK"] not in CSF_attributes:
			UnmatchingForeignKey('CVP_TASK', attributes_dictionary["CVP_TASK"], 'CCF_CNAME or CSF_NAME', 'CCF_table/ CSF_table')
		# CVP_CVSID
		...
		if (int(attributes_dictionary["DPC_COMM"]) < 0 or int(attributes_dictionary["DPC_COMM"]) > 9999) and int(attributes_dictionary["DPC_COMM"]) != 0:
		...
		if attributes_dictionary["DPC_FORM"] not in ['B', 'O', 'D', 'H', 'N'] and attributes_dictionary["DPC_FORM"] != '': 
		...
		# CPC_DISPFMT
		if attributes_dictionary["CPC_DISPFMT"] not in ['A', 'I', 'U', 'R', 'T', 'D'] and attributes_dictionary["CPC_DISPFMT"] != '': 
			InvalidAttribute("CPC_DISPFMT", attributes_dictionary["CPC_DISPFMT"], not_applicable= False)
		if (attributes_dictionary["CPC_CATEG"] == 'A' or attributes_dictionary["CPC_CATEG"] == 'P' or int(attributes_dictionary["CPC_PTC"]) == 11) and attributes_dictionary["CPC_DISPFMT"] != '':
		...
		if attributes_dictionary["CPC_CATEG"] not in ['C', 'T', 'B', 'A', 'P', 'N'] and attributes_dictionary["CPC_CATEG"] != '': 
			InvalidAttribute("CPC_CATEG", attributes_dictionary["CPC_CATEG"], not_applicable= False)
		if attributes_dictionary["CPC_CATEG"] == 'A' and (int(attributes_dictionary["CPC_PTC"]) != 7 or int(attributes_dictionary["CPC_PFC"]) != 0):
		...
		if attributes_dictionary["CDF_VALUE"] != '':
			...
			if CPC_object == None:
				...
				elif CPC_object.CPC_CATEG == 'P': # field shall contain the name of a monitoring parameter (PCF_NAME) with PCF_PID not null
					PCF_rows= session.query(schema.PCF).all() # gets the PCF table
					PCF_attributes= []
					for PCF_object in PCF_rows:
						if PCF_object.PCF_PID != None: # PCF rows with PCF_PID not null
		...
		if attributes_dictionary["CDF_VALUE"] == '':
			if attributes_dictionary["CDF_ELTYPE"] == 'A' or (attributes_dictionary["CDF_ELTYPE"] == 'F' and attributes_dictionary["CDF_INTER"] != 'D') or (attributes_dictionary["CDF_ELTYPE"] == 'E' and attributes_dictionary["CDF_INTER"] == 'T'):
			...
			if attributes_dictionary["CDF_ELTYPE"] == 'E' and attributes_dictionary["CDF_INTER"] != 'T':
				...
				InvalidAttribute("CDF_VALUE", attributes_dictionary["CDF_VALUE"], not_applicable= False, full_message= 'The user must enter a value before loading the command')
		# CDF_TMID
		if attributes_dictionary["CDF_INTER"] == 'T' and attributes_dictionary["CDF_TMID"] != '':
			...
			if PCF_cdf_tmid == None:
				UnmatchingForeignKey("CDF_TMID", attributes_dictionary["CDF_TMID"], "PCF_NAME", "PCF_table")
			else:
				if PCF_cdf_pname == None:
					...
					if PCF_cdf_tmid.PCF_PTC != PCF_cdf_pname.PCF_PTC or PCF_cdf_tmid.PCF_PFC != PCF_cdf_pname.PCF_PFC:
						print("PCF_PTC, PCF_PFC attributes of row %s changed to %s, %s" %( str(PCF_cdf_tmid.PCF_NAME)), str(PCF_cdf_pname.PCF_PTC),  str(PCF_cdf_pname.PCF_PFC) )
		...
		if attributes_dictionary["CVE_CVSID"] not in CVS_attributes:
			UnmatchingForeignKey('CVE_CVSID', attributes_dictionary["CVE_CVSID"], "CVS_ID with CVS_SOURCE = 'V'", 'CVS_table')
		# CVE_PARNAM**********
		PCF_rows= session.query(schema.PCF).all() # gets the PCF table
		PCF_attributes= []
		for PCF_object in PCF_rows:
		...
		if PCF_object == None:
			...
			if attributes_dictionary["CVE_INTER"] == 'E' and PCF_object.PCF_CATEG == 'S' and attributes_dictionary["CVE_TOL"] != '':
		...
		if attributes_dictionary["PRF_DSPFMT"] not in ['A', 'I', 'U', 'R', 'T', 'D'] and attributes_dictionary["PRF_DSPFMT"] != '': 
		...
		if attributes_dictionary["CSP_VTYPE"] == 'E' and attributes_dictionary["CSP_CATEG"] not in ['T', 'C']:
			InvalidAttribute("CSP_VTYPE", attributes_dictionary["CSP_VTYPE"], not_applicable= False)
		if attributes_dictionary["CSP_VTYPE"] == '' and attributes_dictionary["CSP_TYPE"] == 'P' and attributes_dictionary["CSP_DEFVAL"] != '':
			InvalidAttribute("CSP_VTYPE", attributes_dictionary["CSP_VTYPE"], not_applicable= False)
		if attributes_dictionary["CSP_TYPE"] in ['C', 'S'] and attributes_dictionary["CSP_VTYPE"] != '': # must be left null
		...
		if attributes_dictionary["CSP_DEFVAL"] == '':
			# The user must enter a value before loading the command
			InvalidAttribute("CSP_DEFVAL", attributes_dictionary["CSP_DEFVAL"], not_applicable= False, full_message= 'The user must enter a value before loading the command')
		# CSP_PRFREF
		PRF_rows= session.query(schema.PRF).all() # gets the PRF table
		PRF_attributes= []
		for PRF_object in PRF_rows:
		...
		if (attributes_dictionary["CSP_CATEG"] in ['', 'A', 'P', 'N', 'T'] or attributes_dictionary["CSP_TYPE"] in ['C', 'S']) and attributes_dictionary["CSP_CCAREF"] != '':
		...
		if (attributes_dictionary["CSP_CATEG"] in ['', 'A', 'P', 'N', 'C'] or attributes_dictionary["CSP_TYPE"] in ['C', 'S']) and attributes_dictionary["CSP_CCAREF"] != '':
		...
		# CSP_DISPFMT
		if attributes_dictionary["CSP_DISPFMT"] not in ['I', 'U', 'A', 'R', 'T', 'D'] and attributes_dictionary["CSP_DISPFMT"] != '':
		...
		if CCA_object == None:
			...
			if attributes_dictionary["CSP_CATEG"] == 'C' and CCA_object.CCA_ENGFMT == 'I' and attributes_dictionary["CSP_DISPFMT"] != 'I':
				print("CSP_DISPFMT changed to 'I'")
				object_to_update.CSP_DISPFMT= 'I'
			if attributes_dictionary["CSP_CATEG"] == 'C' and CCA_object.CCA_ENGFMT == 'U' and attributes_dictionary["CSP_DISPFMT"] != 'U':
				print("CSP_DISPFMT changed to 'U'")
				object_to_update.CSP_DISPFMT= 'U'
			if attributes_dictionary["CSP_CATEG"] == 'C' and CCA_object.CCA_ENGFMT == 'R' and attributes_dictionary["CSP_DISPFMT"] != 'R':
			...
			if (attributes_dictionary["CSP_CATEG"] == 'A' or attributes_dictionary["CSP_CATEG"] == 'P') and attributes_dictionary["CSP_DISPFMT"] != '':
		...
		if attributes_dictionary["CSP_CATEG"] not in ['C', 'T', 'A', 'P', 'B'] and attributes_dictionary["CSP_CATEG"] != '':
			InvalidAttribute("CSP_CATEG", attributes_dictionary["CSP_CATEG"], not_applicable= False)
		if attributes_dictionary["CSP_CATEG"] == 'A' and (int(attributes_dictionary["CSP_PTC"]) != 7 or int(attributes_dictionary["CSP_PTC"]) != 0):
		...
		if element_type == 'command':
			...
			if attributes_dictionary["SDF_PNAME"] not in CDF_attributes:
				UnmatchingForeignKey('SDF_PNAME', attributes_dictionary["SDF_PNAME"], "CDF_PNAME with CDF_ELTYPE= 'E'", 'CDF_table')
		...
		if element_type == 'command':
			CPC_object= session.query(schema.CPC).filter_by(CPC_PNAME= attributes_dictionary["SDF_PNAME"]).first() # *unchanged*
			CDF_object= session.query(schema.CDF).filter_by(CDF_PNAME= attributes_dictionary["SDF_PNAME"]).first() # *unchanged*
			categ= CPC_object.CPC_CATEG 
			grpsize= int(CDF_object.CDF_GRPSIZE)
		if element_type == 'sequence':
			CSP_object= session.query(schema.CSP).filter_by(CSC_FPNAME= attributes_dictionary["SDF_PNAME"]).first() # *unchanged*
		...
		if attributes_dictionary["SDF_VTYPE"] == 'F' and grpsize > 0:
			InvalidAttribute("SDF_VTYPE", attributes_dictionary["SDF_VTYPE"], not_applicable= False)
		if attributes_dictionary["SDF_VTYPE"] == 'S' and CSP_object.CSP_DEFVAL == None:
			InvalidAttribute("SDF_VTYPE", attributes_dictionary["SDF_VTYPE"], not_applicable= False, full_message= "CSP_DEFVAL cannot be left null.")
		# SDF_VALUE
		...
		for CSP_object in CSP_rows:
			CSP_attributes.append(CSP_object.CSP_FPNAME)
		if attributes_dictionary["PVS_PNAME"] not in CDF_attributes and attributes_dictionary["PVS_PNAME"] not in CSP_attributes:
			UnmatchingForeignKey('PVS_PNAME', attributes_dictionary["PVS_PNAME"], 'CDF_PNAME/ CSP_FPNAME', 'CDF_table/ CSP_table')
		# PVS_INTER
		if attributes_dictionary["PVS_PNAME"] in CDF_attributes:
			CPC_object= session.query(schema.CPC).filter_by(CPC_PNAME= attributes_dictionary["PVS_PNAME"]).first() # *unchanged*
			categ= CPC_object.CPC_CATEG
		if attributes_dictionary["PVS_PNAME"] in CSP_attributes:
			CSP_object= session.query(schema.CSP).filter_by(CSP_FPNAME= attributes_dictionary["PVS_PNAME"]).first() # *unchanged*
			categ= CSP_object.CSP_CATEG
		...
		if (int(attributes_dictionary["VDF_RELEASE"]) < 0 or int(attributes_dictionary["VDF_RELEASE"]) > pow(2, 16)-1) and int(attributes_dictionary["VDF_RELEASE"]) != 0:
			IntegerAttributeOutOfRange("VDF_RELEASE", attributes_dictionary["VDF_RELEASE"])
		if (int(attributes_dictionary["VDF_ISSUE"]) < 0 or int(attributes_dictionary["VDF_ISSUE"]) > pow(2, 16)-1) and int(attributes_dictionary["VDF_RELEASE"]) != 0:
		...
		if (int(attributes_dictionary["PPF_NBPR"]) < 1 or int(attributes_dictionary["PPF_NBPR"]) > 32) and int(attributes_dictionary["PPF_NBPR"]) != 0:
		...
		# PPC_FORM
		if attributes_dictionary["PPC_FORM"] not in ['B', 'O', 'D', 'H', 'N'] and attributes_dictionary["PPC_FORM"] != '': 
		...
		if attributes_dictionary["CPS_PAR"] not in CDF_attributes and attributes_dictionary["CPS_PAR"] not in CSP_attributes:
			UnmatchingForeignKey('CPS_PAR', attributes_dictionary["CPS_PAR"], 'CDF_PNAME/ CSP_NAME', 'CDF_table/ CSP_table')
		# CPS_BIT
...
def setDefaultValues(class_type_str, object_to_update):
	...
	if class_type_str == "DPF":
		...
		if object_to_update.CAF_INTER == None:
		...
		if object_to_update.CCF_CRITICAL == None:
			object_to_update.CCF_CRITICAL = 'N'
		if object_to_update.CCF_PLAN == None:
			object_to_update.CCF_PLAN = 'N'
		if object_to_update.CCF_EXEC == None:
			object_to_update.CCF_EXEC = 'Y'
		if object_to_update.CCF_ILSCOPE == None:
			object_to_update.CCF_ILSCOPE = 'N'
		if object_to_update.CCF_ILSTAGE == None:
			object_to_update.CCF_ILSTAGE = 'C'
		if object_to_update.CCF_HIPRI == None:
			object_to_update.CCF_HIPRI = 'N'
	
	elif class_type_str == "CSF":
		if object_to_update.CSF_IFTT == None:
			object_to_update.CSF_IFTT = 'N'
		if object_to_update.CSF_CRITICAL == None:
			object_to_update.CSF_CRITICAL = 'N'
		if object_to_update.CSF_PLAN == None:
			object_to_update.CSF_PLAN = 'N'
		if object_to_update.CSF_EXEC == None:
			object_to_update.CSF_EXEC = 'Y'

	elif class_type_str == "CSS":
		if object_to_update.CSS_MANDISP == None:
			object_to_update.CSS_MANDISP = 'N'
		if object_to_update.CSS_RELTYPE == None:
			object_to_update.CSS_RELTYPE = 'R'
		if object_to_update.CSS_PREVREL == None:
			object_to_update.CSS_PREVREL = 'R'
		if object_to_update.CSS_DYNPTV == None:
			object_to_update.CSS_DYNPTV = 'N'
		if object_to_update.CSS_STAPTV == None:
			object_to_update.CSS_STAPTV = 'N'
		if object_to_update.CSS_CEV == None:
			object_to_update.CSS_CEV = 'N'

	elif class_type_str == "PAF":
		if object_to_update.PAF_RAWFMT == None:
		...
		if object_to_update.PID_PI1_VAL == None:
			object_to_update.PID_PI1_VAL = 0
		if object_to_update.PID_PI2_VAL == None:
			object_to_update.PID_PI2_VAL = 0
		if object_to_update.PID_TPSD == None:
			object_to_update.PID_TPSD = -1
		if object_to_update.PID_TIME == None:
			object_to_update.PID_TIME = 'N'
		if object_to_update.PID_VALID == None:
			object_to_update.PID_VALID = 'Y'
		if object_to_update.PID_CHECK == None:
			object_to_update.PID_CHECK = 0
		if object_to_update.PID_EVENT == None:
			object_to_update.PID_EVENT = 'N'

	elif class_type_str == "CVS":
		if object_to_update.CVS_UNCERTAINTY == None:
			object_to_update.CVS_UNCERTAINTY = -1

	elif class_type_str == "VPD":
		if object_to_update.VPD_GRPSIZE == None:
			object_to_update.VPD_GRPSIZE = 0
		if object_to_update.VPD_FIXREP == None:
			object_to_update.VPD_FIXREP = 0
		if object_to_update.VPD_CHOICE == None:
			object_to_update.VPD_CHOICE = 'N'
		if object_to_update.VPD_PIDREF == None:
			object_to_update.VPD_PIDREF = 'N'
		if object_to_update.VPD_JUSTIFY == None:
			object_to_update.VPD_JUSTIFY = 'L'
		if object_to_update.VPD_NEWLINE == None:
			object_to_update.VPD_NEWLINE = 'N'
		if object_to_update.VPD_DCHAR == None:
			object_to_update.VPD_DCHAR = 0
		if object_to_update.VPD_FORM == None:
			object_to_update.VPD_FORM = 'N'
		if object_to_update.VPD_OFFSET == None:
			object_to_update.VPD_OFFSET = 0

	elif class_type_str == "PCF":
		if object_to_update.PCF_INTER == None:
			object_to_update.PCF_INTER = 'F'
		if object_to_update.PCF_USCON == None:
			object_to_update.PCF_USCON = 'N'
		if object_to_update.PCF_VALPAR == None:
			object_to_update.PCF_VALPAR = -1
		if object_to_update.PCF_CORR == None:
			object_to_update.PCF_CORR = 'Y'
		if object_to_update.PCF_DARC == None:
			object_to_update.PCF_DARC = '0'
		if object_to_update.PCF_ENDIAN == None:
		...
		if object_to_update.OCP_VALPAR == None:
		...
		if object_to_update.PLF_NBOCC == None:
			object_to_update.PLF_NBOCC = -1
		if object_to_update.PLF_LGOCC == None:
			object_to_update.PLF_LGOCC = 0
		if object_to_update.PLF_TIME == None:
			object_to_update.PLF_TIME = 0
		if object_to_update.PLF_TDOCC == None:
		...
		if object_to_update.SPC_UPDT == None:
			object_to_update.SPC_UPDT = ' '
		if object_to_update.SPC_MODE == None:
			object_to_update.SPC_MODE = ' '
		if object_to_update.SPC_FORM == None:
			object_to_update.SPC_FORM = 'N'
		if object_to_update.SPC_BACK == None:
			object_to_update.SPC_BACK = '0'

	elif class_type_str == "PCPC":
		if object_to_update.PCPC_CODE == None:
			object_to_update.PCPC_CODE = 'U'

	elif class_type_str == "PCDF":
		if object_to_update.PCDF_RADIX == None:
		...
		if object_to_update.PTV_INTER == None:
			object_to_update.PTV_INTER = 'R'

	elif class_type_str == "GPC":
		if object_to_update.GPC_RAW == None:
			object_to_update.GPC_RAW = 'U'
		if object_to_update.GPC_SYMB0 == None:
			object_to_update.GPC_SYMB0 = '0'
		if object_to_update.GPC_LINE == None:
		...
		if object_to_update.CVP_TYPE == None:
			object_to_update.CVP_TYPE = 'C'

	elif class_type_str == "DPC":
		if object_to_update.DPC_COMM == None:
			object_to_update.DPC_COMM = 0
		if object_to_update.DPC_MODE == None:
			object_to_update.DPC_MODE = 'Y'
		if object_to_update.DPC_FORM == None:
			object_to_update.DPC_FORM = 'N'

	elif class_type_str == "CCA":
		if object_to_update.CCA_ENGFMT == None:
			object_to_update.CCA_ENGFMT = 'R'
		if object_to_update.CCA_RAWFMT == None:
			object_to_update.CCA_RAWFMT = 'U'
		if object_to_update.CCA_RADIX == None:
		...
		if object_to_update.CPC_DISPFMT == None:
			object_to_update.CPC_DISPFMT = 'R'
		if object_to_update.CPC_RADIX == None:
			object_to_update.CPC_RADIX = 'D'
		if object_to_update.CPC_CATEG == None:
			object_to_update.CPC_CATEG = 'N'
		if object_to_update.CPC_INTER == None:
			object_to_update.CPC_INTER = 'R'
		if object_to_update.CPC_CORR == None:
			object_to_update.CPC_CORR = 'Y'
		if object_to_update.CPC_OBTID == None:
			object_to_update.CPC_OBTID = 0
		if object_to_update.CPC_ENDIAN == None:
			object_to_update.CPC_ENDIAN = 'B'

	elif class_type_str == "CDF":
		if object_to_update.CDF_GRPSIZE == None:
			object_to_update.CDF_GRPSIZE = 0
		if object_to_update.CDF_INTER == None:
			object_to_update.CDF_INTER = 'R'

	elif class_type_str == "CVE":
		if object_to_update.CVE_INTER == None:
			object_to_update.CVE_INTER = 'R'
		if object_to_update.CVE_CHECK == None:
			object_to_update.CVE_CHECK = 'B'

	elif class_type_str == "PRF":
		if object_to_update.PRF_INTER == None:
			object_to_update.PRF_INTER = 'R'
		if object_to_update.PRF_DSPFMT == None:
			object_to_update.PRF_DSPFMT = 'U'
		if object_to_update.PRF_RADIX == None:
		...
		if object_to_update.CSP_DISPFMT == None:
			object_to_update.CSP_DISPFMT = 'R'
		if object_to_update.CSP_RADIX == None:
			object_to_update.CSP_RADIX = 'D'
		if object_to_update.CSP_CATEG == None:
			object_to_update.CSP_CATEG = 'N'

	elif class_type_str == "SDF":
		if object_to_update.SDF_FTYPE == None:
			object_to_update.SDF_FTYPE = 'E'

	elif class_type_str == "PVS":
		if object_to_update.PVS_INTER == None:
			object_to_update.PVS_INTER = 'R'

	elif class_type_str == "VDF":
		if object_to_update.VDF_RELEASE == None:
			object_to_update.VDF_RELEASE = 0
		if object_to_update.VDF_ISSUE == None:
			object_to_update.VDF_ISSUE = 0

	elif class_type_str == "MCF":
		if object_to_update.MCF_POL2 == None:
			object_to_update.MCF_POL2 = '0'
		if object_to_update.MCF_POL3 == None:
			object_to_update.MCF_POL3 = '0'
		if object_to_update.MCF_POL4 == None:
			object_to_update.MCF_POL4 = '0'
		if object_to_update.MCF_POL5 == None:
			object_to_update.MCF_POL5 = '0'
		
	elif class_type_str == "LGF":
		if object_to_update.LGF_POL2 == None:
			object_to_update.LGF_POL2 = '0'
		if object_to_update.LGF_POL3 == None:
			object_to_update.LGF_POL3 = '0'
		if object_to_update.LGF_POL4 == None:
			object_to_update.LGF_POL4 = '0'
		if object_to_update.LGF_POL5 == None:
			object_to_update.LGF_POL5 = '0'

	elif class_type_str == "PIC":
		if object_to_update.PIC_APID == None:
		...
		if object_to_update.PPC_FORM == None:
