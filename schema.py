...
class CUR(Base):
	...
	CUR_POS = Column(Integer, CheckConstraint("CUR_POS < %d" %sys.maxsize), CheckConstraint('LENGTH("CUR_POS") <= 2'), primary_key= True, nullable= False)
	CUR_RLCHK = Column(String(8), CheckConstraint('LENGTH("CUR_RLCHK") <= 8'), nullable= False)
	CUR_VALPAR = Column(Integer, CheckConstraint("CUR_VALPAR < %d" %sys.maxsize), CheckConstraint('LENGTH("CUR_VALPAR") <= 5'), nullable= False)
	CUR_SELECT = Column(String(10), CheckConstraint('LENGTH("CUR_SELECT") <= 10'), nullable= False) # ForeignKey('LGF_table.LGF_IDENT'), ForeignKey('MCF_table.MCF_IDENT'), ForeignKey('CAF_table.CAF_NUMBR')

	# LGF_rel= relationship("LGF", uselist= False, back_populates= 'CUR_rel')
...
class PLF(Base):
	...
	PLF_NAME = Column(String(8), CheckConstraint('LENGTH("PLF_NAME") <= 8'), ForeignKey('PCF_table.PCF_NAME'), primary_key= True, nullable= False)
	PLF_SPID = Column(Integer, CheckConstraint("PLF_SPID < %d" %sys.maxsize), CheckConstraint('LENGTH("PLF_SPID") <= 10'), ForeignKey('PID_table.PID_SPID'), primary_key= True, nullable= False)
	PLF_OFFBY = Column(Integer, CheckConstraint("PLF_OFFBY < %d" %sys.maxsize), CheckConstraint('LENGTH("PLF_OFFBY") <= 5'), nullable= False)
	PLF_OFFBI = Column(Integer, CheckConstraint("PLF_OFFBI < %d" %sys.maxsize), CheckConstraint('LENGTH("PLF_OFFBI") <= 1'), nullable= False)
	PLF_NBOCC = Column(Integer, CheckConstraint("PLF_NBOCC < %d" %sys.maxsize), CheckConstraint('LENGTH("PLF_NBOCC") <= 4'))
	PLF_LGOCC = Column(Integer, CheckConstraint("PLF_LGOCC < %d" %sys.maxsize), CheckConstraint('LENGTH("PLF_LGOCC") <= 5'))
	PLF_TIME = Column(Integer, CheckConstraint("PLF_TIME < %d" %sys.maxsize), CheckConstraint('LENGTH("PLF_TIME") <= 9'))
	PLF_TDOCC = Column(Integer, CheckConstraint("PLF_TDOCC < %d" %sys.maxsize), CheckConstraint('LENGTH("PLF_TDOCC") <= 9'))
...
class PID(Base):
	...
	PID_TYPE = Column(Integer, CheckConstraint("PID_TYPE < %d" %sys.maxsize), CheckConstraint('LENGTH("PID_TYPE") <= 3'), primary_key= True, nullable= False)
	PID_STYPE = Column(Integer, CheckConstraint("PID_STYPE < %d" %sys.maxsize), CheckConstraint('LENGTH("PID_STYPE") <= 3'), primary_key= True, nullable= False)
	PID_APID = Column(Integer, CheckConstraint("PID_APID < %d" %sys.maxsize), CheckConstraint('LENGTH("PID_APID") <= 5'), primary_key= True, nullable= False)
	PID_PI1_VAL = Column(Integer, CheckConstraint("PID_PI1_VAL < %d" %sys.maxsize), CheckConstraint('LENGTH("PID_PI1_VAL") <= 10'), primary_key= True)
	PID_PI2_VAL = Column(Integer, CheckConstraint("PID_PI2_VAL < %d" %sys.maxsize), CheckConstraint('LENGTH("PID_PI2_VAL") <= 10'), primary_key= True)
	PID_SPID = Column(Integer, CheckConstraint("PID_SPID < %d" %sys.maxsize), CheckConstraint('LENGTH("PID_SPID") <= 10'), primary_key= True, nullable= False)
	...
	PID_TPSD = Column(Integer, CheckConstraint("PID_TPSD < %d" %sys.maxsize), CheckConstraint('LENGTH("PID_TPSD") <= 10'))
	PID_DFHSIZE = Column(Integer, CheckConstraint("PID_DFHSIZE < %d" %sys.maxsize), CheckConstraint('LENGTH("PID_DFHSIZE") <= 2'), nullable= False)
	PID_TIME = Column(String(1), CheckConstraint('LENGTH("PID_TIME") <= 1'))
	PID_INTER = Column(Integer, CheckConstraint("PID_INTER < %d" %sys.maxsize), CheckConstraint('LENGTH("PID_INTER") <= 8'))
	PID_VALID = Column(String(1), CheckConstraint('LENGTH("PID_VALID") <= 1'))
	PID_CHECK = Column(Integer, CheckConstraint("PID_CHECK < %d" %sys.maxsize), CheckConstraint('LENGTH("PID_CHECK") <= 1'))
...
class TPCF(Base):
	...
	TPCF_SPID = Column(Integer, CheckConstraint("TPCF_SPID < %d" %sys.maxsize), CheckConstraint('LENGTH("TPCF_SPID") <= 10'), ForeignKey('PID_table.PID_SPID'), primary_key= True, nullable= False)
	TPCF_NAME = Column(String(12), CheckConstraint('LENGTH("TPCF_NAME") <= 12'))
	TPCF_SIZE = Column(Integer, CheckConstraint("TPCF_SIZE < %d" %sys.maxsize), CheckConstraint('LENGTH("TPCF_SIZE") <= 8')) # field will not be used by SCOS-2000
...
class CAF(Base):
	...
	CAF_NCURVE = Column(Integer, CheckConstraint("CAF_NCURVE < %d" %sys.maxsize), CheckConstraint('LENGTH("CAF_NCURVE") <= 3')) # field will not be used by SCOS-2000
	...
	def __repr__(self):
		return """
			...
			CAF_UNIT= %s
			CAF_NCURVE= %s
			CAF_INTER= %s""" % (str(self.CAF_NUMBR), str(self.CAF_DESCR), str(self.CAF_ENGFMT), str(self.CAF_RAWFMT),
...
class TXF(Base):
	...
	TXF_NALIAS = Column(Integer, CheckConstraint("TXF_NALIAS < %d" %sys.maxsize), CheckConstraint('LENGTH("TXF_NALIAS") <= 3'))
...
class OCF(Base):
	...
	OCF_NAME = Column(String(8), CheckConstraint('LENGTH("OCF_NAME") <= 8'), ForeignKey('PCF_table.PCF_NAME'), primary_key= True, nullable= False)
	OCF_NBCHCK = Column(Integer, CheckConstraint("OCF_NBCHCK < %d" %sys.maxsize), CheckConstraint('LENGTH("OCF_NBCHCK") <= 2'), nullable= False)
	OCF_NBOOL = Column(Integer, CheckConstraint("OCF_NBOOL < %d" %sys.maxsize), CheckConstraint('LENGTH("OCF_NBOOL") <= 2'), nullable= False)
...
class CAP(Base):
	...
	CAP_NUMBR = Column(String(10), CheckConstraint('LENGTH("CAP_NUMBR") <= 10'), ForeignKey('CAF_table.CAF_NUMBR'), nullable= False, primary_key= True)
...
class TXP(Base):
	...
	TXP_NUMBR = Column(String(10), CheckConstraint('LENGTH("TXP_NUMBR") <= 10'), ForeignKey('TXF_table.TXF_NUMBR'), nullable= False, primary_key= True)
...
class OCP(Base):
	...
	OCP_NAME = Column(String(8), CheckConstraint('LENGTH("OCP_NAME") <= 8'), ForeignKey('OCF_table.OCF_NAME'), nullable= False, primary_key= True)
	OCP_POS = Column(Integer, CheckConstraint("OCP_POS < %d" %sys.maxsize), CheckConstraint('LENGTH("OCP_POS") <= 2'), nullable= False, primary_key= True) # field will not be used by SCOS-2000
	...
	OCP_VALPAR = Column(Integer, CheckConstraint("OCP_VALPAR < %d" %sys.maxsize), CheckConstraint('LENGTH("OCP_VALPAR") <= 5'))
...
class GRPA(Base):
	...
	GRPA_GNAME = Column(String(14), CheckConstraint('LENGTH("GRPA_GNAME") <= 14'), ForeignKey('GRP_table.GRP_NAME'), primary_key= True, nullable= False)
...
class GRPK(Base):
	...
	GRPK_GNAME = Column(String(14), CheckConstraint('LENGTH("GRPK_GNAME") <= 14'), ForeignKey('GRP_table.GRP_NAME'), primary_key= True, nullable= False)
	GRPK_PKSPID = Column(Integer, CheckConstraint("GRPK_PKSPID < %d" %sys.maxsize), CheckConstraint('LENGTH("GRPK_PKSPID") <= 10'), ForeignKey("PID_table.PID_SPID"), primary_key= True, nullable= False)
...
class PCF(Base):
	...
	PCF_PTC = Column(Integer, CheckConstraint("PCF_PTC < %d" %sys.maxsize), CheckConstraint('LENGTH("PCF_PTC") <= 2'), nullable= False)
	PCF_PFC = Column(Integer, CheckConstraint("PCF_PFC < %d" %sys.maxsize), CheckConstraint('LENGTH("PCF_PFC") <= 5'), nullable= False)
	PCF_WIDTH = Column(Integer, CheckConstraint("PCF_WIDTH < %d" %sys.maxsize), CheckConstraint('LENGTH("PCF_WIDTH") <= 6'))
	...
	PCF_CURTX = Column(String(8), CheckConstraint('LENGTH("PCF_CURTX") <= 8')) # ForeignKey('TXF_table.TXF_NUMBR'), ForeignKey('CAF_table.CAF_NUMBR'), ForeignKey('MCF_table.MCF_NUMBR'), ForeignKey('LGF_table.LGF_NUMBR')
	PCF_INTER = Column(String(1), CheckConstraint('LENGTH("PCF_INTER") <= 1')) # field will not be used by SCOS-2000 
	PCF_USCON = Column(String(1), CheckConstraint('LENGTH("PCF_USCON") <= 1'))
	PCF_DECIM = Column(Integer, CheckConstraint("PCF_DECIM < %d" %sys.maxsize), CheckConstraint('LENGTH("PCF_DECIM") <= 3'))
	PCF_PARVAL = Column(String(14), CheckConstraint('LENGTH("PCF_PARVAL") <= 14'))
	PCF_SUBSYS = Column(String(8), CheckConstraint('LENGTH("PCF_SUBSYS") <= 8')) # field will not be used by SCOS-2000 
	PCF_VALPAR = Column(Integer, CheckConstraint("PCF_VALPAR < %d" %sys.maxsize), CheckConstraint('LENGTH("PCF_VALPAR") <= 5'))
	PCF_SPTYPE = Column(String(1), CheckConstraint('LENGTH("PCF_SPTYPE") <= 1')) # field will not be used by SCOS-2000
	PCF_CORR = Column(String(1), CheckConstraint('LENGTH("PCF_CORR") <= 1'))
	PCF_OBTID = Column(Integer, CheckConstraint("PCF_OBTID < %d" %sys.maxsize), CheckConstraint('LENGTH("PCF_OBTID") <= 5'))
...
class DPC(Base):
	...
	DPC_NUMBE = Column(String(8), CheckConstraint('LENGTH("DPC_NUMBE") <= 8'), ForeignKey('DPF_table.DPF_NUMBE'), primary_key= True, nullable= False)
	DPC_NAME = Column(String(8), CheckConstraint('LENGTH("DPC_NAME") <= 8'), ForeignKey('PCF_table.PCF_NAME'))
	DPC_FLDN = Column(Integer, CheckConstraint("DPC_FLDN < %d" %sys.maxsize), CheckConstraint('LENGTH("DPC_FLDN") <= 2'), primary_key= True, nullable= False)
	DPC_COMM = Column(Integer, CheckConstraint("DPC_COMM < %d" %sys.maxsize), CheckConstraint('LENGTH("DPC_COMM") <= 4'))
	...
	def __repr__(self):
		return """
			...
			DPC_MODE = %s
			DPC_FORM = %s
			DPC_TEXT = %s """ % (str(self.DPC_NUMBE), str(self.DPC_NAME), str(self.DPC_FLDN), str(self.DPC_COMM), str(self.DPC_MODE), str(self.DPC_FORM), str(self.DPC_TEXT))
...
class GPC(Base):
	...
	GPC_NUMBE = Column(String(8), CheckConstraint('LENGTH("GPC_NUMBE") <= 8'), ForeignKey('GPF_table.GPF_NUMBE'), primary_key= True, nullable= False)
	GPC_POS = Column(Integer, CheckConstraint("GPC_POS < %d" %sys.maxsize), CheckConstraint('LENGTH("GPC_POS") <= 1'), primary_key= True, nullable= False) # field will not be used by SCOS-2000
	GPC_WHERE = Column(String(1), CheckConstraint('LENGTH("GPC_WHERE") <= 1'), nullable= False)
	GPC_NAME = Column(String(8), CheckConstraint('LENGTH("GPC_NAME") <= 8'), ForeignKey('PCF_table.PCF_NAME'), nullable= False)
	...
	GPC_DOMAIN = Column(Integer, CheckConstraint("GPC_DOMAIN < %d" %sys.maxsize), CheckConstraint('LENGTH("GPC_DOMAIN") <= 5'))
...
class SPC(Base):
	...
	SPC_NUMBE = Column(String(8), CheckConstraint('LENGTH("SPC_NUMBE") <= 8'), ForeignKey("SPF_table.SPF_NUMBE"), primary_key= True, nullable= False)
	SPC_POS =Column(Integer, CheckConstraint("SPC_POS < %d" %sys.maxsize), CheckConstraint('LENGTH("SPC_POS") <= 1'), primary_key= True, nullable= False) # field will not be used by SCOS-2000 
	SPC_NAME = Column(String(8), CheckConstraint('LENGTH("SPC_NAME") <= 8'), ForeignKey('PCF_table.PCF_NAME'), nullable= False)
...
class VPD(Base):
	...
	VPD_TPSD = Column(Integer, CheckConstraint("VPD_TPSD < %d" %sys.maxsize), CheckConstraint('LENGTH("VPD_TPSD") <= 10'), ForeignKey('PID_table.PID_TPSD'), primary_key= True, nullable= False)
	VPD_POS = Column(Integer, CheckConstraint("VPD_POS < %d" %sys.maxsize), CheckConstraint('LENGTH("VPD_POS") <= 4'), primary_key= True, nullable= False) # field will not be used by SCOS-2000
	VPD_NAME = Column(String(8), CheckConstraint('LENGTH("VPD_NAME") <= 8'), ForeignKey('PCF_table.PCF_NAME'), nullable= False)
	VPD_GRPSIZE = Column(Integer, CheckConstraint("VPD_GRPSIZE < %d" %sys.maxsize), CheckConstraint('LENGTH("VPD_GRPSIZE") <= 3'))
	VPD_FIXREP = Column(Integer, CheckConstraint("VPD_FIXREP < %d" %sys.maxsize), CheckConstraint('LENGTH("VPD_FIXREP") <= 3'))
	...
	VPD_WIDTH = Column(Integer, CheckConstraint("VPD_WIDTH < %d" %sys.maxsize), CheckConstraint('LENGTH("VPD_WIDTH") <= 2'), nullable= False)
	...
	VPD_DCHAR = Column(Integer, CheckConstraint("VPD_DCHAR < %d" %sys.maxsize), CheckConstraint('LENGTH("VPD_DCHAR") <= 1'))
	VPD_FORM = Column(String(1), CheckConstraint('LENGTH("VPD_FORM") <= 1'))
	VPD_OFFSET = Column(Integer, CheckConstraint("VPD_OFFSET < %d" %sys.maxsize), CheckConstraint('LENGTH("VPD_OFFSET") <= 6'))
...
class DPF(Base):
	...
	def __repr__(self):
		return """
			DPF_NUMBE = %s
			DPF_TYPE = %s
			DPF_HEAD = %s""" % (self.DPF_NUMBE, self.DPF_TYPE, self.DPF_HEAD)
...
class GPF(Base):
	...
	GPF_DAYS = Column(Integer, CheckConstraint("GPF_DAYS < %d" %sys.maxsize), CheckConstraint('LENGTH("GPF_DAYS") <= 2'), nullable= False)
	GPF_HOURS = Column(Integer, CheckConstraint("GPF_HOURS < %d" %sys.maxsize), CheckConstraint('LENGTH("GPF_HOURS") <= 2'), nullable= False)
	GPF_MINUT = Column(Integer, CheckConstraint("GPF_MINUT < %d" %sys.maxsize), CheckConstraint('LENGTH("GPF_MINUT") <= 2'), nullable= False)
	GPF_AXCLR = Column(String(1), CheckConstraint('LENGTH("GPF_AXCLR") <= 1'), nullable= False)
	GPF_XTICK = Column(Integer, CheckConstraint("GPF_XTICK < %d" %sys.maxsize), CheckConstraint('LENGTH("GPF_XTICK") <= 2'), nullable= False)
	GPF_YTICK = Column(Integer, CheckConstraint("GPF_YTICK < %d" %sys.maxsize), CheckConstraint('LENGTH("GPF_YTICK") <= 2'), nullable= False)
	GPF_XGRID = Column(Integer, CheckConstraint("GPF_XGRID < %d" %sys.maxsize), CheckConstraint('LENGTH("GPF_XGRID") <= 2'), nullable= False)
	GPF_YGRID = Column(Integer, CheckConstraint("GPF_YGRID < %d" %sys.maxsize), CheckConstraint('LENGTH("GPF_YGRID") <= 2'), nullable= False)
	GPF_UPUN = Column(Integer, CheckConstraint("GPF_UPUN < %d" %sys.maxsize), CheckConstraint('LENGTH("GPF_UPUN") <= 2')) # field will not be used by SCOS-2000
...
class SPF(Base):
	...
	SPF_NPAR = Column(Integer, CheckConstraint("SPF_NPAR < %d" %sys.maxsize), CheckConstraint('LENGTH("SPF_NPAR") <= 1'), nullable= False)
	SPF_UPUN = Column(Integer, CheckConstraint("SPF_UPUN < %d" %sys.maxsize), CheckConstraint('LENGTH("SPF_UPUN") <= 2')) # field will not be used by SCOS-2000
...
class PTV(Base):
	...
	PTV_CNAME = Column(String(8), CheckConstraint('LENGTH("PTV_CNAME") <= 8'), ForeignKey('CCF_table.CCF_CNAME'), primary_key= True, nullable= False)
...
class PCDF (Base):
	...
	PCDF_TCNAME = Column(String(8), CheckConstraint('LENGTH("PCDF_TCNAME") <= 8'), ForeignKey("TCP_table.TCP_ID"), primary_key= True, nullable= False)
	...
	PCDF_LEN = Column(Integer, CheckConstraint("PCDF_LEN < %d" %sys.maxsize), CheckConstraint('LENGTH("PCDF_LEN") <= 4'), nullable= False)
	PCDF_BIT = Column(Integer, CheckConstraint("PCDF_BIT < %d" %sys.maxsize), CheckConstraint('LENGTH("PCDF_BIT") <= 4'), primary_key= True, nullable= False)
...
class CVP(Base):
	...
	CVP_TASK = Column(String(8), CheckConstraint('LENGTH("CVP_TASK") <= 8'), ForeignKey('CCF_table.CCF_CNAME'), primary_key= True, nullable= False)
	CVP_TYPE = Column(String(1), CheckConstraint('LENGTH("CVP_TYPE") <= 1'), primary_key= True)
	CVP_CVSID = Column(Integer, CheckConstraint("CVP_CVSID < %d" %sys.maxsize), CheckConstraint('LENGTH("CVP_CVSID") <= 5'), ForeignKey('CVS_table.CVS_ID'), primary_key= True, nullable= False)
...
class CVS(Base):
	...
	CVS_ID = Column(Integer, CheckConstraint("CVS_ID < %d" %sys.maxsize), CheckConstraint('LENGTH("CVS_ID") <= 5'), primary_key= True, nullable= False)
	...
	CVS_START = Column(Integer, CheckConstraint("CVS_START < %d" %sys.maxsize), CheckConstraint('LENGTH("CVS_START") <= 5'), nullable= False)
	CVS_INTERVAL = Column(Integer, CheckConstraint("CVS_INTERVAL < %d" %sys.maxsize), CheckConstraint('LENGTH("CVS_INTERVAL") <= 5'), nullable= False)
	CVS_SPID = Column(Integer, CheckConstraint("CVS_SPID < %d" %sys.maxsize), CheckConstraint('LENGTH("CVS_SPID") <= 10')) # field will not be used by SCOS-2000
	CVS_UNCERTAINTY = Column(Integer, CheckConstraint("CVS_UNCERTAINTY < %d" %sys.maxsize), CheckConstraint('LENGTH("CVS_UNCERTAINTY") <= 10'))
...
class CVE(Base):
	...
	CVE_CVSID = Column(Integer, CheckConstraint("CVE_CVSID < %d" %sys.maxsize), CheckConstraint('LENGTH("CVE_CVSID") <= 5'), ForeignKey('CVS_table.CVS_ID'), primary_key= True, nullable= False)
...
class CSS(Base):
	...
	CSS_SQNAME = Column(String(8), CheckConstraint('LENGTH("CSS_SQNAME") <= 8'), ForeignKey("CSF_table.CSF_NAME"), primary_key= True, nullable= False)
	CSS_COMM = Column(String(32), CheckConstraint('LENGTH("CSS_COMM") <= 32'))
	CSS_ENTRY = Column(Integer, CheckConstraint("CSS_ENTRY < %d" %sys.maxsize), CheckConstraint('LENGTH("CSS_ENTRY") <= 5'), primary_key= True, nullable= False)
	CSS_TYPE = Column(String(1), CheckConstraint('LENGTH("CSS_TYPE") <= 1'), nullable= False)
	CSS_ELEMID = Column(String(8), CheckConstraint('LENGTH("CSS_ELEMID") <= 8')) # ForeignKey("CCF_table.CCF_CNAME"), ForeignKey("CSF_table.CSF_NAME")
	CSS_NPARS = Column(Integer, CheckConstraint("CSS_NPARS < %d" %sys.maxsize), CheckConstraint('LENGTH("CSS_NPARS") <= 3')) # field will not be used by SCOS-2000
...
class CCF(Base):
	...
	CCF_DESCR2 = Column(String(64), CheckConstraint('LENGTH("CCF_DESCR2") <= 64')) # field will not be used by SCOS-2000
	...
	CCF_PKTID = Column(String(8), CheckConstraint('LENGTH("CCF_PKTID") <= 8'), ForeignKey('TCP_table.TCP_ID'), nullable= False)
	CCF_TYPE = Column(Integer, CheckConstraint("CCF_TYPE < %d" %sys.maxsize), CheckConstraint('LENGTH("CCF_TYPE") <= 3'))
	CCF_STYPE = Column(Integer, CheckConstraint("CCF_STYPE < %d" %sys.maxsize), CheckConstraint('LENGTH("CCF_STYPE") <= 3'))
	CCF_APID = Column(Integer, CheckConstraint("CCF_APID < %d" %sys.maxsize), CheckConstraint('LENGTH("CCF_APID") <= 5'))
	CCF_NPARS = Column(Integer, CheckConstraint("CCF_NPARS < %d" %sys.maxsize), CheckConstraint('LENGTH("CCF_NPARS") <= 3')) # field will not be used by SCOS-2000
	...
	CCF_SUBSYS = Column(Integer, CheckConstraint("CCF_SUBSYS < %d" %sys.maxsize), CheckConstraint('LENGTH("CCF_SUBSYS") <= 3'))
	CCF_HIPRI = Column(String(1), CheckConstraint('LENGTH("CCF_HIPRI") <= 1'))
	CCF_MAPID = Column(Integer, CheckConstraint("CCF_MAPID < %d" %sys.maxsize), CheckConstraint('LENGTH("CCF_MAPID") <= 2'))
	CCF_DEFSET = Column(String(8), CheckConstraint('LENGTH("CCF_DEFSET") <= 8'), ForeignKey('PSV_table.PSV_PVSID'))
	CCF_RAPID = Column(Integer, CheckConstraint("CCF_RAPID < %d" %sys.maxsize), CheckConstraint('LENGTH("CCF_RAPID") <= 5'))
	CCF_ACK = Column(Integer, CheckConstraint("CCF_ACK < %d" %sys.maxsize), CheckConstraint('LENGTH("CCF_ACK") <= 2'))
	CCF_SUBSCHEDID = Column(Integer, CheckConstraint("CCF_SUBSCHEDID < %d" %sys.maxsize), CheckConstraint('LENGTH("CCF_SUBSCHEDID") <= 5'))
...
class CSF(Base):
	...
	CSF_NFPARS = Column(Integer, CheckConstraint("CSF_NFPARS < %d" %sys.maxsize), CheckConstraint('LENGTH("CSF_NFPARS") <= 3'))
	CSF_ELEMS = Column(Integer, CheckConstraint("CSF_ELEMS < %d" %sys.maxsize), CheckConstraint('LENGTH("CSF_ELEMS") <= 5')) # field will not be used by SCOS-2000
	...
	CSF_SUBSYS = Column(Integer, CheckConstraint("CSF_SUBSYS < %d" %sys.maxsize), CheckConstraint('LENGTH("CSF_SUBSYS") <= 3'))
	CSF_GENTIME = Column(String(17), CheckConstraint('LENGTH("CSF_GENTIME") <= 17')) # field will not be used by SCOS-2000
	CSF_DOCNAME = Column(String(32), CheckConstraint('LENGTH("CSF_DOCNAME") <= 32'))# field will not be used by SCOS-2000
	...
	CSF_SUBSCHEDID = Column(Integer, CheckConstraint("CSF_SUBSCHEDID < %d" %sys.maxsize), CheckConstraint('LENGTH("CSF_SUBSCHEDID") <= 5'))
...
class CDF(Base):
	...
	CDF_CNAME = Column(String(8), CheckConstraint('LENGTH("CDF_CNAME") <= 8'), ForeignKey('CCF_table.CCF_CNAME'), primary_key= True, nullable= False)
	...
	CDF_ELLEN = Column(Integer, CheckConstraint("CDF_ELLEN < %d" %sys.maxsize), CheckConstraint('LENGTH("CDF_ELLEN") <= 4'), nullable= False)
	CDF_BIT = Column(Integer, CheckConstraint("CDF_BIT < %d" %sys.maxsize), CheckConstraint('LENGTH("CDF_BIT") <= 4'), primary_key= True, nullable= False)
	CDF_GRPSIZE = Column(Integer, CheckConstraint("CDF_GRPSIZE < %d" %sys.maxsize), CheckConstraint('LENGTH("CDF_GRPSIZE") <= 2'))
	CDF_PNAME = Column(String(8), CheckConstraint('LENGTH("CDF_PNAME") <= 8'), ForeignKey('CPC_table.CPC_PNAME')) # any other foreign key?
...
class PSV(Base):
	...
	PSV_NAME = Column(String(8), CheckConstraint('LENGTH("PSV_NAME") <= 8'), ForeignKey('PST_table.PST_NAME'), nullable= False)
...
class PVS(Base):
	...
	PVS_ID = Column(String(8), CheckConstraint('LENGTH("PVS_ID") <= 8'), ForeignKey('PSV_table.PSV_PVSID'), primary_key= True, nullable= False)
	PVS_PSID = Column(String(8), CheckConstraint('LENGTH("PVS_PSID") <= 8'), nullable= False)
	PVS_PNAME = Column(String(8), CheckConstraint('LENGTH("PVS_PNAME") <= 8'), nullable= False) # ForeignKey('CDF_table.CDF_PNAME'),  ForeignKey('CSP_table.CSP_FPNAME')
	PVS_INTER = Column(String(1), CheckConstraint('LENGTH("PVS_INTER") <= 1'))
	PVS_VALS = Column(String)
	PVS_BIT = Column(Integer, CheckConstraint("PVS_BIT < %d" %sys.maxsize), CheckConstraint('LENGTH("PVS_BIT") <= 4'), primary_key= True, nullable= False)
...
class PSM(Base):
	...
	PSM_NAME = Column(String(8), CheckConstraint('LENGTH("PSM_NAME") <= 8'), primary_key= True, nullable= False) # ForeignKey('CSF_table.CSF_NAME'), ForeignKey('CCF_table.CCF_CNAME')
	PSM_TYPE = Column(String(1), CheckConstraint('LENGTH("PSM_TYPE") <= 1'), primary_key= True, nullable= False)
	PSM_PARSET = Column(String(8), CheckConstraint('LENGTH("PSM_PARSET") <= 8'), ForeignKey('PST_table.PST_NAME'), primary_key= True, nullable= False)
...
class SDF(Base):
	...
	SDF_ENTRY = Column(Integer, CheckConstraint("SDF_ENTRY < %d" %sys.maxsize), CheckConstraint('LENGTH("SDF_ENTRY") <= 5'), primary_key= True, nullable= False)
	SDF_ELEMID = Column(String(8), CheckConstraint('LENGTH("SDF_ELEMID") <= 8'), nullable= False)
	SDF_POS = Column(Integer, CheckConstraint("SDF_POS < %d" %sys.maxsize), CheckConstraint('LENGTH("SDF_POS") <= 4'), primary_key= True, nullable= False)
	SDF_PNAME = Column(String(8), CheckConstraint('LENGTH("SDF_PNAME") <= 8'), primary_key= True, nullable= False) # ForeignKey('CSP_table.CSP_FPNAME'), ForeignKey('CDF_table.CDF_PNAME')
	...
	SDF_REPPOS = Column(Integer, CheckConstraint("SDF_REPPOS < %d" %sys.maxsize), CheckConstraint('LENGTH("SDF_REPPOS") <= 4'))
...
class CSP(Base):
	...
	CSP_SQNAME = Column(String(8), CheckConstraint('LENGTH("CSP_SQNAME") <= 8'), ForeignKey("CSF_table.CSF_NAME"), primary_key= True, nullable= False)
	CSP_FPNAME = Column(String(8), CheckConstraint('LENGTH("CSP_FPNAME") <= 8'), primary_key= True, nullable= False)
	CSP_FPNUM = Column(Integer, CheckConstraint("CSP_FPNUM < %d" %sys.maxsize), CheckConstraint('LENGTH("CSP_FPNUM") <= 5'), nullable= False)
	CSP_DESCR = Column(String(24), CheckConstraint('LENGTH("CSP_DESCR") <= 24'))
	CSP_PTC = Column(Integer, CheckConstraint("CSP_PTC < %d" %sys.maxsize), CheckConstraint('LENGTH("CSP_PTC") <= 2'), nullable= False)
	CSP_PFC = Column(Integer, CheckConstraint("CSP_PFC < %d" %sys.maxsize), CheckConstraint('LENGTH("CSP_PFC") <= 5'), nullable= False)
...
class CPC(Base):
	...
	CPC_PTC = Column(Integer, CheckConstraint("CPC_PTC < %d" %sys.maxsize), CheckConstraint('LENGTH("CPC_PTC") <= 2'), primary_key= True, nullable= False)
	CPC_PFC = Column(Integer, CheckConstraint("CPC_PFC < %d" %sys.maxsize), CheckConstraint('LENGTH("CPC_PFC") <= 5'), primary_key= True, nullable= False)
	...
	CPC_OBTID = Column(Integer, CheckConstraint("CPC_OBTID < %d" %sys.maxsize), CheckConstraint('LENGTH("CPC_OBTID") <= 5'))
...
class CCA(Base):
	...
	CCA_NCURVE = Column(Integer, CheckConstraint("CCA_NCURVE < %d" %sys.maxsize), CheckConstraint('LENGTH("CCA_NCURVE") <= 3')) # field will not be used by SCOS-2000
...
class PAF(Base):
	...
	PAF_NALIAS = Column(Integer, CheckConstraint("PAF_NALIAS < %d" %sys.maxsize), CheckConstraint('LENGTH("PAF_NALIAS") <= 3')) # field will not be used by SCOS-2000
...
class PRF(Base):
	...
	PRF_NRANGE = Column(Integer, CheckConstraint("PRF_NRANGE < %d" %sys.maxsize), CheckConstraint('LENGTH("PRF_NRANGE") <= 3')) # field will not be used by SCOS-2000
...
class CCS(Base):
	...
	CCS_NUMBR = Column(String(10), CheckConstraint('LENGTH("CCS_NUMBR") <= 10'), ForeignKey('CCA_table.CCA_NUMBR'), primary_key= True, nullable= False)
...
class PAS(Base):
	...
	PAS_NUMBR = Column(String(10), CheckConstraint('LENGTH("PAS_NUMBR") <= 10'), ForeignKey("PAF_table.PAF_NUMBR"), primary_key= True, nullable= False)
...
class PRV(Base):
	...
	PRV_NUMBR = Column(String(10), CheckConstraint('LENGTH("PRV_NUMBR") <= 10'), ForeignKey('PRF_table.PRF_NUMBR'), primary_key= True, nullable= False)
...
class CPS(Base): # table not required for the processing within SCOS-2000 
	...
	CPS_BIT = Column(Integer, CheckConstraint("CPS_BIT < %d" %sys.maxsize), CheckConstraint('LENGTH("CPS_BIT") <= 4'), primary_key= True, nullable= False)
...
class DST(Base): # optional table
	...
	DST_APID = Column(Integer, CheckConstraint("DST_APID < %d" %sys.maxsize), CheckConstraint('LENGTH("DST_APID") <= 5'), primary_key= True, nullable= False)
...
class PIC(Base):
	...
	PIC_TYPE = Column(Integer, CheckConstraint("PIC_TYPE < %d" %sys.maxsize), CheckConstraint('LENGTH("PIC_TYPE") <= 3'), primary_key= True, nullable= False)
	PIC_STYPE = Column(Integer, CheckConstraint("PIC_STYPE < %d" %sys.maxsize), CheckConstraint('LENGTH("PIC_STYPE") <= 3'), primary_key= True, nullable= False)
	PIC_PI1_OFF = Column(Integer, CheckConstraint("PIC_PI1_OFF < %d" %sys.maxsize), CheckConstraint('LENGTH("PIC_PI1_OFF") <= 5'), nullable= False)
	PIC_PI1_WID = Column(Integer, CheckConstraint("PIC_PI1_WID < %d" %sys.maxsize), CheckConstraint('LENGTH("PIC_PI1_WID") <= 3'), nullable= False)
	PIC_PI2_OFF = Column(Integer, CheckConstraint("PIC_PI2_OFF < %d" %sys.maxsize), CheckConstraint('LENGTH("PIC_PI2_OFF") <= 5'), nullable= False)
	PIC_PI2_WID = Column(Integer, CheckConstraint("PIC_PI2_WID < %d" %sys.maxsize), CheckConstraint('LENGTH("PIC_PI2_WID") <= 3'), nullable= False)
	PIC_APID = Column(Integer, CheckConstraint("PIC_APID < %d" %sys.maxsize), CheckConstraint('LENGTH("PIC_APID") <= 5')) # primary_key ??
...
class VDF(Base):
	...
	VDF_DOMAINID = Column(Integer, CheckConstraint("VDF_DOMAINID < %d" %sys.maxsize), CheckConstraint('LENGTH("VDF_DOMAINID") <= 5'))
	VDF_RELEASE = Column(Integer, CheckConstraint("VDF_RELEASE < %d" %sys.maxsize), CheckConstraint('LENGTH("VDF_RELEASE") <= 5'))
	VDF_ISSUE = Column(Integer, CheckConstraint("VDF_ISSUE < %d" %sys.maxsize), CheckConstraint('LENGTH("VDF_ISSUE") <= 5'))
...
class PPF(Base): # table not required for the processing within SCOS-2000 
	...
	PPF_NBPR = Column(Integer, CheckConstraint("PPF_NBPR < %d" %sys.maxsize), CheckConstraint('LENGTH("PPF_NBPR") <= 2'))
...
class PPC(Base): # table not required for the processing within SCOS-2000 
	...
	PPC_POS = Column(Integer, CheckConstraint("PPC_POS < %d" %sys.maxsize), CheckConstraint('LENGTH("PPC_POS") <= 32'), nullable= False, primary_key= True)
...
tablename_dict= {'CSS': CSS, 'CCF': CCF, 'CSF': CSF, 'PSM': PSM, 'PSV': PSV, 'SDF': SDF, # dictionary associating table names with classes
...
not_supported_tables= ['SCO', 'TCD', 'TMD'] # tables which are not supported by the application

suggested_input_order_dict= {} # dictionary associating each table with the order in which it should be added to the database
i= 0
