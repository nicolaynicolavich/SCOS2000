...
class mainFrame (wx.Frame):
	...
	def __init__(self, parent):
		...
		wx.Frame.__init__ ( self, parent, id = wx.ID_ANY, title = u"MIB database manager", pos = wx.DefaultPosition, size = wx.Size( 566,331 ), style = wx.CAPTION|wx.CLOSE_BOX|wx.MINIMIZE|wx.TAB_TRAVERSAL )
		...
		self.m_button11 = wx.Button( self.m_panel9, wx.ID_ANY, u"Create\nnew\ndatabase", wx.DefaultPosition, wx.DefaultSize, 0 )
		self.m_button11.SetFont( wx.Font( 24, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, "Arial" ) )
		#left button size
		...
		self.openDatabase_button.SetFont( wx.Font( 24, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, "Arial" ) )
		#right button size
	...
	def createNewDatabase(self):
		...
		# engine = sqlalchemy.create_engine('sqlite:///' + imported_database_path.replace('\\', '/') + '/database.db', echo=False)
...
class workspaceFrame ( wx.Frame ):
	...
	def __init__( self, parent ):
		...
		wx.Frame.__init__ ( self, parent, id = wx.ID_ANY, title = u"MIB database manager", pos = wx.DefaultPosition, size = wx.Size( 930,607 ), style = wx.DEFAULT_FRAME_STYLE|wx.TAB_TRAVERSAL )
		...
		self.importedLocation_menuItem = wx.MenuItem( self.file_menu, wx.ID_ANY, u"Open different database", wx.EmptyString, wx.ITEM_NORMAL )
		...
		self.save_menuItem = wx.MenuItem( self.save_menu, wx.ID_ANY, u"Save session"+ u"\t" + u"CTRL+S", wx.EmptyString, wx.ITEM_NORMAL )
		self.save_menu.Append( self.save_menuItem )

		# self.saveDatabase_menuItem = wx.MenuItem( self.save_menu, wx.ID_ANY, u"Save database file", wx.EmptyString, wx.ITEM_NORMAL )
		# self.save_menu.Append( self.saveDatabase_menuItem )
		
		self.savingLocation_menuItem = wx.MenuItem( self.save_menu, wx.ID_ANY, u"Change saving location", wx.EmptyString, wx.ITEM_NORMAL )
		...
		self.version_menuItem = wx.MenuItem( self.about_menu, wx.ID_ANY, u"Version", wx.EmptyString, wx.ITEM_NORMAL )
		self.about_menu.Append( self.version_menuItem )

		self.tableOrder_menuItem = wx.MenuItem( self.about_menu, wx.ID_ANY, u"Table input order", wx.EmptyString, wx.ITEM_NORMAL )
		...
		self.add_row_menuItem = wx.MenuItem( self.actions_menu, wx.ID_ANY, u"Add row"+ u"\t" + u"CTRL+A", wx.EmptyString, wx.ITEM_NORMAL )
		self.actions_menu.Append( self.add_row_menuItem )
		
		self.edit_row_menuItem = wx.MenuItem( self.actions_menu, wx.ID_ANY, u"Edit row"+ u"\t" + u"CTRL+E", wx.EmptyString, wx.ITEM_NORMAL )
		self.actions_menu.Append( self.edit_row_menuItem )
		
		self.delete_row_menuItem = wx.MenuItem( self.actions_menu, wx.ID_ANY, u"Delete Row"+ u"\t" + u"CTRL+D", wx.EmptyString, wx.ITEM_NORMAL )
		...
		# Show tables
		self.table_tab_container = wx.aui.AuiNotebook( self.workspace_panel, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.aui.AUI_NB_TAB_MOVE|wx.aui.AUI_NB_TOP|wx.aui.AUI_NB_WINDOWLIST_BUTTON|wx.aui.AUI_NB_SCROLL_BUTTONS )
		self.panel_tab= {}
		self.table_bSizer = {}
		self.table_grid = {}
		self.max_width_dict = {}
		self.tableTab_event_Id_dict= {} # dictionary associating the event id of each table with its corresponding class
		...
		for table in sorted(tablename_list):
			if table not in not_supported_tables:
				...
				self.panel_tab.update({ table: wx.Panel( self.table_tab_container, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.TAB_TRAVERSAL )})
				...
				self.table_grid.update({ table : wx.grid.Grid( self.panel_tab[table], wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, 0 ) })
...
class addFrame (wx.Frame):
	...
	def __init__(self, parent, **kwargs):
		...
		wx.Frame.__init__(self, parent, id=wx.ID_ANY, title = u"MIB database manager", pos=wx.DefaultPosition, size=wx.Size(
			1000, 500), style=wx.DEFAULT_FRAME_STYLE | wx.TAB_TRAVERSAL)

		self.class_attributes, self.nullable_key_dict, self.foreign_keys_dict = functions.getColumnNames(self.class_type)
		...
		self.m_menuItem2 = wx.MenuItem( self.view_menu, wx.ID_ANY, u"Datatype and maximum length", wx.EmptyString, wx.ITEM_CHECK )
		self.view_menu.Append( self.m_menuItem2 )
		self.m_menuItem2.Check( True )
		
		self.m_menuItem21 = wx.MenuItem( self.view_menu, wx.ID_ANY, u"Enable/ Disable field override", wx.EmptyString, wx.ITEM_CHECK )
		self.view_menu.Append( self.m_menuItem21 )
		self.m_menuItem21.Check( False )

		self.m_menuItem3 = wx.MenuItem( self.view_menu, wx.ID_ANY, u"Enable/ Disable field description", wx.EmptyString, wx.ITEM_CHECK )
		...
		self.header_label = wx.StaticText( self.header_panel, wx.ID_ANY, u"Enter the following parameters. Once you are done, click the OK button on the right.", wx.DefaultPosition, wx.DefaultSize, 0 )
		self.header_label.Wrap( -1 )
		self.header_label.SetFont( wx.Font( 12, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
		...
		self.OK_button.SetFont( wx.Font( 12, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
		...
		self.input_data_scrolledWindow = wx.ScrolledWindow( self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.VSCROLL )
		...
		override_attribute_dict= {} # dictionary indicating whether to overrride a certain field
		self.field_label = []
		self.value_textCtrl = []
		self.datatype_label = []
		self.checkOverride = []
		self.mouse_hover_Id_dict = {} # dictionary associating the event Id of each mouse hover event with its correspondig attribute
		i = 0
		for attribute in self.class_attributes:
			if self.nullable_key_dict[attribute] == False:
				self.field_label.append(wx.StaticText(self.input_data_scrolledWindow, wx.ID_ANY, attribute, wx.DefaultPosition, wx.DefaultSize, 0))
				self.field_label[i].Wrap(-1)
				self.field_label[i].SetFont( wx.Font( 9, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial Black" ) )
			else:
				self.field_label.append(wx.StaticText(self.input_data_scrolledWindow, wx.ID_ANY, attribute, wx.DefaultPosition, wx.DefaultSize, 0))
				self.field_label[i].Wrap(-1)
				self.field_label[i].SetFont( wx.Font( 9, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
			...
			if self.foreign_keys_dict[attribute] == None: # attribute is not a foreign key
				if kwargs['mode'] == 'edit': # displays the current attributes of the object
					if kwargs['object_attributes'][i] not in [None, '-']:
						...
						current_val= ''
					self.value_textCtrl.append(wx.TextCtrl(self.input_data_scrolledWindow, wx.ID_ANY, current_val, wx.DefaultPosition, wx.DefaultSize, 0))
				elif kwargs['mode'] == 'add':
					self.value_textCtrl.append(wx.TextCtrl(self.input_data_scrolledWindow, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition, wx.DefaultSize, 0))
				...
				if kwargs['mode'] == 'edit': # displays the current attributes of the object
					if kwargs['object_attributes'][i] not in [None, '-']:
						...
						current_val= ''
					self.value_textCtrl.append(wx.ComboBox( self.input_data_scrolledWindow, wx.ID_ANY, current_val, wx.DefaultPosition, wx.DefaultSize, list(set(FK_comboBoxChoices)), 0 ))
				if kwargs['mode'] == 'add':
					self.value_textCtrl.append(wx.ComboBox( self.input_data_scrolledWindow, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition, wx.DefaultSize, list(set(FK_comboBoxChoices)), 0 ))
			...
			# print(self.datatype_label_text) # unknown why this does not show the same text as on GUI panel
			self.datatype_label.append(wx.StaticText( self.input_data_scrolledWindow, wx.ID_ANY, self.datatype_label_text, wx.DefaultPosition, wx.DefaultSize, 0 ))
			self.datatype_label[i].Wrap( -1 )
			input_data_fgSizer.Add( self.datatype_label[i], 0, wx.ALL, 5 )
			self.checkOverride.append(wx.CheckBox( self.input_data_scrolledWindow, wx.ID_ANY, u"Override", wx.DefaultPosition, wx.DefaultSize, 0 ))
		...
		self.OK_button.Bind(wx.EVT_BUTTON, self.onOKButton); self.mode = kwargs['mode']
	...
	def promptTable(self, *args):
		...
		self.m_staticText79.SetFont(wx.Font(14, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, "Arial Black"))
		...
		for item in sorted(tablename_list):
			self.m_button.append(wx.Button(self.m_scrolledWindow3, wx.ID_ANY, item, wx.DefaultPosition, wx.DefaultSize, 0))
	...
	def onOKButton(self, event):
		...
		if self.mode == 'add':
			...
			for key in self.class_attributes:  # update the object with the data the user has input
				...
				i += 1
			registerAttributes(self, self.class_attributes, self.class_type, mode= self.mode, object_to_edit= object_to_edit)
	...
	def openInfo(self, event):
		...
		if self.enable_description == True:
			...
			self.info_dialog= infoDialog(self, "%s field description" % attribute, field_description_dict[class_type_str][attribute])
...
class updatesuccessfulDialog ( wx.Dialog ):
	...
	def __init__( self, parent):
		...
		wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = u"Update successful", pos = wx.DefaultPosition, size = wx.Size( 342,182 ), style = wx.DEFAULT_DIALOG_STYLE|wx.STAY_ON_TOP )
		...
		self.m_staticText8 = wx.StaticText( self.m_panel2, wx.ID_ANY, u"Update successful!", wx.DefaultPosition, wx.DefaultSize, wx.ALIGN_CENTRE )
		self.m_staticText8.Wrap( -1 )
		self.m_staticText8.SetFont( wx.Font( 22, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, "Arial" ) )
		...
		self.addAgain_button = wx.Button( self.m_panel3, wx.ID_ANY, u"Add another row", wx.DefaultPosition, wx.DefaultSize, 0 )
		self.addAgain_button.SetFont( wx.Font( 12, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
		...
		self.cancel_button.SetFont( wx.Font( 12, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
...
class updateFailedDialog ( wx.Dialog ):
	...
	def __init__( self, parent, error_message ):
		wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = u"Update failed", pos = wx.DefaultPosition, size = wx.Size( 482,324 ), style = wx.DEFAULT_DIALOG_STYLE )
		...
		self.m_staticText15 = wx.StaticText( self, wx.ID_ANY, u"Unable to update database", wx.DefaultPosition, wx.DefaultSize, 0 )
		self.m_staticText15.Wrap( -1 )
		self.m_staticText15.SetFont( wx.Font( 20, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, "Arial" ) )
		
		fgSizer6.Add( self.m_staticText15, 0, wx.ALL|wx.ALIGN_CENTER_HORIZONTAL, 5 )
		
		self.m_staticText16 = wx.StaticText( self, wx.ID_ANY, "The following error occured: " + error_message, wx.DefaultPosition, wx.DefaultSize, 0 )
		self.m_staticText16.Wrap( -1 )
		self.m_staticText16.SetFont( wx.Font( 11, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
		...
		self.m_button15.SetFont( wx.Font( 11, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
...
class databaseSavedDialog ( wx.Dialog ):
	...
	def __init__( self, parent ):
		wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = u"Database saved", pos = wx.DefaultPosition, size = wx.Size( 406,201 ), style = wx.DEFAULT_DIALOG_STYLE )
		
		self.SetSizeHints( wx.DefaultSize, wx.DefaultSize )
		
		bSizer2 = wx.BoxSizer( wx.VERTICAL )
		
		self.m_staticText5 = wx.StaticText( self, wx.ID_ANY, u"Database saved successfully", wx.DefaultPosition, wx.DefaultSize, 0 )
		self.m_staticText5.Wrap( -1 )
		self.m_staticText5.SetFont( wx.Font( 16, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, "Arial" ) )
		
		bSizer2.Add( self.m_staticText5, 1, wx.ALL|wx.ALIGN_CENTER_HORIZONTAL, 5 )
		
		self.m_staticText6 = wx.StaticText( self, wx.ID_ANY, "Saved at %s" % saved_database_path, wx.DefaultPosition, wx.DefaultSize, 0 )
...
class importDatabaseDialog ( wx.Dialog ):
	...
	def __init__( self, parent ):
		wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = u"Import Database", pos = wx.DefaultPosition, size = wx.Size( 461,285 ), style = wx.CAPTION|wx.CLOSE_BOX|wx.STAY_ON_TOP )
		...
		self.m_staticText7 = wx.StaticText( self.m_panel2, wx.ID_ANY, u"Enter the location (folder) of the desired database, then click OK:", wx.DefaultPosition, wx.DefaultSize, 0 )
		self.m_staticText7.Wrap( -1 )
		self.m_staticText7.SetFont( wx.Font( 14, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
		...
		self.location_picker = wx.DirPickerCtrl( self.m_panel3, wx.ID_ANY, wx.EmptyString, u"Select a folder", wx.DefaultPosition, wx.DefaultSize, wx.DIRP_DEFAULT_STYLE )
		...
		# self.db_button = wx.Button( self.m_panel4, wx.ID_ANY, u"Database file (db)", wx.DefaultPosition, wx.DefaultSize, 0 )
		# self.db_button.SetFont( wx.Font( 12, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
		...
		self.txt_button.SetFont( wx.Font( 12, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
...
class savingLocationDialog ( wx.Dialog ):
	...
	def __init__( self, parent ):
		wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = u"Saving location", pos = wx.DefaultPosition, size = wx.Size( 461,285 ), style = wx.CAPTION|wx.CLOSE_BOX|wx.STAY_ON_TOP )
		...
		self.m_staticText7 = wx.StaticText( self.m_panel2, wx.ID_ANY, u"Enter the location where the database is to be saved:", wx.DefaultPosition, wx.DefaultSize, 0 )
		self.m_staticText7.Wrap( -1 )
		self.m_staticText7.SetFont( wx.Font( 14, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
		...
		self.location_picker = wx.DirPickerCtrl( self.m_panel3, wx.ID_ANY, wx.EmptyString, u"Select a folder", wx.DefaultPosition, wx.DefaultSize, wx.DIRP_USE_TEXTCTRL )
		bSizer6.Add( self.location_picker, 1, wx.ALL|wx.EXPAND, 5 )
		...
		self.save_button = wx.Button( self.m_panel4, wx.ID_ANY, u"Change location", wx.DefaultPosition, wx.DefaultSize, 0 )
		self.save_button.SetFont( wx.Font( 18, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
...
class deleteRowDialog ( wx.Dialog ):
	...
	def __init__( self, parent ):
		...
		wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = u"Delete row", pos = wx.DefaultPosition, size = wx.Size( 440,267 ), style = wx.CAPTION|wx.STAY_ON_TOP )
	...
	def proceed( self, event ):
		registerAttributes(None, self.parent.selected_class_attributes, self.parent.tab_class_type, self.parent.selected_object_attributes, mode= 'delete')
		self.Destroy()
...
class infoDialog ( wx.Dialog ):
	...
	def __init__( self, parent, Title, info ):
		wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = Title, pos = wx.DefaultPosition, size = wx.Size( 500,350 ), style = wx.CAPTION|wx.CLOSE_BOX|wx.MINIMIZE_BOX )
		...
		self.m_staticText14 = wx.StaticText( self.m_scrolledWindow3, wx.ID_ANY, info, wx.DefaultPosition, wx.DefaultSize, 0 )
		self.m_staticText14.Wrap( 390 )
		self.m_staticText14.SetFont( wx.Font( 10, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Arial" ) )
...
class progressDialog ( wx.Dialog ):
	...
	def __init__( self, parent, max_progress, label ):
		...
		wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = u"work in progress", pos = wx.DefaultPosition, size = wx.Size( 417,187 ), style = wx.DEFAULT_DIALOG_STYLE )
		...
		self.label_text.SetFont( wx.Font( 13, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, wx.EmptyString ) )
		
		gSizer10.Add( self.label_text, 0, wx.ALL|wx.ALIGN_CENTER_HORIZONTAL, 5 )
		
		self.progress_bar = wx.Gauge( self, wx.ID_ANY, max_progress, wx.DefaultPosition, wx.DefaultSize, wx.GA_HORIZONTAL|wx.GA_SMOOTH )
...
def registerAttributes(current_add_frame, class_attributes, class_type, *args, **kwargs):
	...
	if kwargs['mode'] == 'add_fromTXT':
		...
		success, error_message= updateDatabase(object_to_change, class_type, class_attributes, object_attributes, mode= 'add')
	elif kwargs['mode'] == 'edit':
		success, error_message= updateDatabase(kwargs['object_to_edit'], class_type, class_attributes, object_attributes, mode= 'edit')
		# update all tables, as deleting a parent object can delet subsequent children.
		for table in tablename_list:
			if table not in not_supported_tables:
				if workspace_frame.table_grid[table].GetNumberRows() != session.query(tablename_dict[table]).count(): # only do if number of rows has changed
...
def updateDatabase(object_to_add, class_type_noformat, class_attributes, object_attributes, **kwargs):
	...
	try:
		if kwargs['mode'] == 'add':
			...
			# session.delete(object_to_add)
			# session.commit()
			object_to_add_updated= functions.checkRuntimeConstraints(object_to_add, class_type_noformat, class_attributes, object_attributes, session, override_attribute_dict) # Checks runtime constraints
			...
			object_to_add_corrected= functions.checkRuntimeConstraints(object_to_add_updated, class_type_noformat, class_attributes, object_attributes, session, override_attribute_dict) # Checks runtime constraints
			# update row
			update_dict= {}
